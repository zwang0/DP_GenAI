...
class UNet(nn.Module):
    def __init__(self, device, c_in = 3, c_out = 3, time_dim = 256, img_size=32):
...
class UNet2(nn.Module):
    def __init__(self, device, c_in, c_out, img_size):
        super(UNet2, self).__init__()
        self.unet1 = UNet(device=device, c_in=c_in, c_out=c_out, img_size=img_size)
        self.unet2 = UNet(device=device, c_in=c_in, c_out=c_out, img_size=img_size)

    def forward(self, x, t):
        return self.unet2(self.unet1(x, t), t)